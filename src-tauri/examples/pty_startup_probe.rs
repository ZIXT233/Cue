...
fn main() -> Result<(), Box<dyn std::error::Error>> {
    #[cfg(windows)]
    {
        ...
        let arch = if std::env::consts::ARCH == "aarch64" { "win-arm64" } else { "win-x64" };
        let dll = std::path::Path::new(env!("CARGO_MANIFEST_DIR"))
            .join("resources/conpty").join(arch).join("conpty.dll");
        let wide: Vec<u16> = dll.as_os_str().encode_wide().chain(Some(0)).collect();
        #[link(name = "kernel32")]
        extern "system" { fn LoadLibraryW(path: *const u16) -> *mut std::ffi::c_void; }
    ...
    let mut startup_handshake = std::env::args().any(|arg| arg == "--startup-handshake")
        .then(conpty_handshake::StartupHandshake::default);
    let cli: Vec<String> = std::env::args().skip_while(|arg| arg != "--").skip(1).collect();
    ...
    if cli.is_empty() { command.args(["/d", "/c", "echo QUE_PTY_READY"]); }
    else { command.args(&cli[1..]); }
    command.env("TERM", "xterm-256color");
    command.env_remove("NO_COLOR");
    command.env("COLORTERM", "truecolor");
    command.env("COLORFGBG", if std::env::args().any(|arg| arg == "--light") { "0;15" } else { "15;0" });
    ...
    while start.elapsed() < Duration::from_secs(20) {
        if theme_cycle && ((phase == 0 && start.elapsed().as_secs() >= 12) || (phase == 1 && start.elapsed().as_secs() >= 16)) {
            phase += 1;
            phase_starts.push(output.len());
            writer.write_all(if phase == 1 { b"\x1b[?997;1n" } else { b"\x1b[?997;2n" })?;
            writer.flush()?;
            println!("THEME_PHASE {phase}");
        }
        let Ok(chunk) = rx.recv_timeout(Duration::from_secs(1)) else { continue };
        let text = String::from_utf8_lossy(&chunk);
        if let Some(reply) = startup_handshake.as_mut().and_then(|h| h.feed(&chunk)) {
            writer.write_all(reply)?;
            writer.flush()?;
            println!("{}ms startup handshake answered", start.elapsed().as_millis());
        }
        if cli.is_empty() || chunk.len() < 80 {
            println!("{}ms {:?}", start.elapsed().as_millis(), text);
        } else {
            println!("{}ms output_bytes={}", start.elapsed().as_millis(), chunk.len());
        }
        ...
        if reply_colors {
            for (i, code) in [10, 11].iter().enumerate() {
                while let Some(offset) = output[color_offsets[i]..].find(&format!("\x1b]{code};?")) {
                    ...
                    let color = if phase == 1 { "0000/2b2b/3636" } else { "fdfd/f6f6/e3e3" };
        ...
        if output.contains("QUE_PTY_READY") {
            println!("READY {}ms cpr={reply} da1={reply_da1}", start.elapsed().as_millis());
            break;
        }
        if !observe_theme && !cli.is_empty() && output.len() > 2000 {
            println!("OUTPUT_2KB {}ms cpr={reply} da1={reply_da1}", start.elapsed().as_millis());
            break;
        }
    }
    println!("theme_subscription={} osc10_query={} osc11_query={}", output.contains("\x1b[?2031h"), output.contains("\x1b]10;?"), output.contains("\x1b]11;?"));
    ...
    for tail in output.split("\x1b[").skip(1) {
        if let Some(end) = tail.find(|c: char| c.is_ascii_alphabetic()) {
            if tail.as_bytes()[end] == b'm' { sgr.insert(&tail[..end]); }
        }
    }
    println!("SGR={sgr:?}");
    for (i, begin) in phase_starts.iter().enumerate() {
        ...
        for tail in output[*begin..end].split("\x1b[").skip(1) {
            if let Some(end) = tail.find('m') {
                ...
                if sequence.len() < 80 && sequence.contains("48;") { backgrounds.insert(sequence); }
            }
        }
        println!("PHASE_{i}_BACKGROUNDS={backgrounds:?}");
    }
    if let Ok(path) = std::env::var("QUE_PTY_PROBE_OUTPUT") { std::fs::write(path, &output)?; }
    let _ = child.kill();
    Ok(())
}
