...
pub async fn prepare_shell(workspace: &QueueWorkspace, directory: &Path, bin_dir: &Path, powershell: bool) -> AppResult<ShellLaunch> {
    ...
    if remote {
        ...
        root = user_home.join(".cache/topcard/shell").join(directory.file_name().unwrap_or_default());
    }
    ...
    if !remote && cfg!(windows) && powershell {
        ...
        args = vec!["-NoLogo".into(), "-NoExit".into(), "-EncodedCommand".into(), encoded];
    } else if name == "zsh" {
        ...
        files.insert(".zprofile".into(), "[[ -r \"$TOPCARD_USER_ZDOTDIR/.zprofile\" ]] && source \"$TOPCARD_USER_ZDOTDIR/.zprofile\"\n".into());
        files.insert(".zshrc".into(), format!(
            "ZDOTDIR=\"$TOPCARD_USER_ZDOTDIR\"\n[[ -r \"$ZDOTDIR/.zshrc\" ]] && source \"$ZDOTDIR/.zshrc\"\nunset TOPCARD_USER_ZDOTDIR\nsource {}\n",
...
pub fn probe_chunk(data: &str) -> Option<(bool, Option<i32>)> {
    ...
    if let Some(index) = data.find("\x1b]133;D") {
        ...
        let code = rest.chars().take_while(|c| c.is_ascii_digit()).collect::<String>().parse().ok();
