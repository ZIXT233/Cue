...
pub async fn save_terminal_images(cwd: &str, images: &[Value]) -> AppResult<Vec<String>> {
    ...
    let directory = if let Some(remote) = &remote {
        let dir = String::from_utf8_lossy(&ssh_exec(&remote.ssh_host, "umask 077; mktemp -d /tmp/topcard-images-XXXXXXXX").await?).trim().to_string();
...
pub async fn save_terminal_files(cwd: &str, files: &[(String, Vec<u8>)]) -> AppResult<Vec<String>> {
    let remote = load_ssh_workspace(cwd)?;
    let directory = if let Some(remote) = &remote {
        let dir = String::from_utf8_lossy(&ssh_exec(&remote.ssh_host, "umask 077; mktemp -d /tmp/topcard-files-XXXXXXXX").await?).trim().to_string();
        if !regex::Regex::new(r"^/tmp/topcard-files-[a-zA-Z0-9]+$").unwrap().is_match(&dir) {
            return Err(AppError::msg("Invalid remote upload directory"));
        }
        dir
    } else {
        tempfile::Builder::new()
            .prefix("topcard-files-")
            .tempdir()
            .map_err(|e| AppError::msg(e.to_string()))?
            .keep()
            .to_string_lossy()
