...
export async function machineRequest(body: object, signal?: AbortSignal) {
  ...
  if (!response.ok) throw Object.assign(new Error(data.code || data.error || 'REQUEST_FAILED'), { code: data.code, prompt: data.prompt, detail: data.detail, raw: !data.code });
  return data;
}
export function RemoteHostEditor({ host, onSaved, onCancel, onBack, hideTitle = false }: { host?: RemoteHost; onSaved: (host: RemoteHost) => void; onCancel: () => void; onBack?: () => void; hideTitle?: boolean }) {
  ...
  return <><form className="remote-host-editor" onSubmit={async e => {
    ...
    {!!error && <p className="machine-error" role="alert">{machineErrorText(error, t)}</p>}
    <div className={`machine-test-status${testError ? ' is-error' : testPassed ? ' is-success' : ''}`} role={testError ? 'alert' : 'status'} title={testError ? machineErrorText(testError, t) : undefined}>{testError ? machineErrorText(testError, t) : testPassed ? t('machines.connectionReady') : ''}</div>
...
export function RemoteHostsSettings() {
  ...
  const [hostTests, setHostTests] = useState<Record<string, { ok: boolean; error?: unknown }>>({});
  const [authHost, setAuthHost] = useState<RemoteHost | null>(null);
  const [authError, setAuthError] = useState<unknown>(null);
  const auth = useSshAuthChallenge();
  const refresh = async () => { const r = await fetch('/api/workspace-machines'); const data = await r.json(); if (!r.ok) throw Object.assign(new Error(data.code || data.error || 'REQUEST_FAILED'), { code: data.code, prompt: data.prompt, detail: data.detail, raw: !data.code }); setHosts(data.hosts); };
  ...
  return <><ConfigPanelShell embedded title={t('machines.settings')} onClose={() => {}}><ConfigDetail><ConfigDetailStack className="remote-host-settings">
    ...
    {editing ? <RemoteHostEditor host={editing === 'new' ? undefined : editing} onCancel={() => setEditing(null)} onSaved={() => { setEditing(null); void refresh().catch(setError); }} /> : <div>
      {hosts.map(host => {
        ...
        const testErrorLabel = testResult?.error ? machineErrorText(testResult.error, t) : undefined;
