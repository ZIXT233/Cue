...
function shellEnvironment(): Record<string, string> {
  ...
  // Que terminals are interactive PTYs. A parent launcher (including
...
export function createTerminal(cwd: string, cols: number, rows: number, id: string = randomUUID(), launch?: TerminalLaunch): string {
  ...
  try {
    ...
    throw new Error(
      ...
      "The binary may be missing or incompatible. In the Que installation directory " +
      ...
      `Then restart Que. Original error: ${error instanceof Error ? error.message : String(error)}`,
      { cause: error },
    );
  }
