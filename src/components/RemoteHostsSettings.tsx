...
export function RemoteHostsSettings() {
  ...
  const testHost = async (host: RemoteHost, password?: string, trustedPrompt?: string) => {
    ...
    catch (e) { setHosts(current => current.map(item => item.id === host.id ? { ...item, connected: false } : item)); window.dispatchEvent(new Event('cue-remote-hosts-changed')); if (auth.present(e)) setAuthError(e); else { setHostTests(current => ({ ...current, [host.id]: { ok: false, errorKey: machineErrorKey(e) } })); setAuthHost(null); } }
